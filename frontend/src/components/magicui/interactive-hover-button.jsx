...
export const InteractiveHoverButton = React.forwardRef(({ children, className, ...props }, ref) => {
  return (
    <button
      ...
      <div
        className="flex absolute top-0 z-10 gap-2 justify-center items-center w-full h-full opacity-0 transition-all duration-300 translate-x-12 text-primary-foreground group-hover:-translate-x-5 group-hover:opacity-100">
        <span>{children}</span>
        <ArrowRight />
      </div>
    </button>
